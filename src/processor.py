...
class Samprocessor:
    ...
    def __call__(self, image: PIL.Image, original_size: tuple, prompt: list) -> dict:
        ...
        inputs = {"image": image_torch, "original_size": original_size, "boxes": box_torch, "prompt": prompt}
    ...
    def process_image(self, image: PIL.Image, original_size: tuple) -> torch.tensor:
        ...
        nd_image = np.expand_dims(nd_image, axis=2)
        nd_image = np.repeat(nd_image, 3, axis=2)
